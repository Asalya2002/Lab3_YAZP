// Lab3_YAZP.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
#include <iostream>
...
class List {
private:
    // Описываем внутренний класс Node
    class Node {
    public:
        X data;
        Node* next;
...
public:
    ...
    void DelLast() {
        if (headList == nullptr || headList == tailList) {
            DelHead();
        }
        else {
            Node* temp = headList;
...
int main() {
    ...
    myList.PrintList(); // Output: 0 1 3 2 4

    myList.toEnd();
    myList.DelCurrent();

    myList.PrintList(); // Output: 0 1 3 2

    myList.Del(1);

    myList.PrintList(); // Output: 0 3 2

    myList.Clear();

    myList.PrintList(); // Output:
